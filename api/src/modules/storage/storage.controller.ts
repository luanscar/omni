...
3. Use como "mediaId" ao enviar mensagem
  ...
  @ApiResponse({
    status: 201,
    description:
      'Arquivo enviado com sucesso. Retorna objeto Media com id, fileName, size, etc.',
  })
  @ApiResponse({
    status: 400,
...
3. Use os IDs para enviar múltiplas mensagens
  ...
  @ApiResponse({
    status: 201,
    description:
      'Arquivos enviados com sucesso. Retorna array de objetos Media.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retorna lista de todos os arquivos enviados pelo seu tenant, ordenados por data de upload (mais recentes primeiro).',
  })
  @ApiResponse({
    status: 200,
    description:
      'Lista de arquivos com id, fileName, size, mimeType, createdAt, etc.',
  })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  @ApiResponse({ status: 403, description: 'Proibido.' })
  findAll(@Request() req) {
...
A URL é válida por tempo limitado (presigned URL).
  ...
  @ApiOperation({
    ...
    description:
      '⚠️ **ATENÇÃO:** Esta ação é irreversível! O arquivo será removido permanentemente do storage e do banco de dados.',
  })
  @ApiResponse({ status: 200, description: 'Arquivo removido com sucesso.' })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  @ApiResponse({
    status: 403,
    description: 'Proibido - Apenas ADMIN e MANAGER.',
  })
  @ApiResponse({ status: 404, description: 'Arquivo não encontrado.' })
