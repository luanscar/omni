...
export async function useDatabaseAuthState(
    ...
    if (existingSession && existingSession.sessionData) {
        ...
        keys = JSON.parse(JSON.stringify(sessionData.keys || {}), BufferJSON.reviver);

        logger.log(`Credenciais carregadas do banco de dados para canal ${channelId}`);
    } else {
    ...
    const saveCreds = async () => {
        try {
            ...
            logger.debug(`Credenciais salvas no banco de dados para canal ${channelId}`);
        } catch (error) {
            logger.error(`Erro ao salvar credenciais para canal ${channelId}:`, error);
            throw error;
        }
    };
