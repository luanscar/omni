import { IsEnum, IsNotEmpty, IsOptional, IsString, IsUUID, IsNumber, IsObject, IsBoolean, ValidateIf } from 'class-validator';
...
class ReactionMessageDto {
    ...
    @ApiProperty({
        description: 'ID da mensagem que receberá a reação (UUID do banco de dados, não providerId)',
...
export class CreateMessageDto {
    ...
    @ApiProperty({
        enum: MessageType,
        description: 'Tipo da mensagem: TEXT, IMAGE, VIDEO, AUDIO, DOCUMENT, STICKER, LOCATION, CONTACT, REACTION',
    ...
    @ApiProperty({
        description: '⚠️ ID da mídia previamente enviada via POST /storage/upload. Obrigatório para IMAGE, VIDEO, AUDIO, DOCUMENT e STICKER',
        required: false,
    ...
    @ApiProperty({
        description: 'ID da mensagem que está sendo respondida (cria um Reply/Quote)',
        required: false,
        example: 'fa49178f-6595-40b9-a569-3d5c07925555'
    })
    @IsOptional()
    ...
    @ApiProperty({
        description: 'Se true, adiciona "*Nome do Agente:*\\n" no início da mensagem',
        required: false,
        default: false,
        example: false
    })
    ...
    @ApiProperty({
        ...
        required: false,
        description: 'Dados do contato compartilhado (obrigatório quando type = CONTACT)',
        example: {
    ...
    @ApiProperty({
        ...
        required: false,
        description: '⚠️ Dados da reação (obrigatório quando type = REACTION). Reações NÃO são salvas no banco, apenas enviadas ao WhatsApp.',
        example: {
            text: '❤️',
